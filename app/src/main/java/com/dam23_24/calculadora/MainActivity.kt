...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var btndDelete: Button
    ...
    private fun initComponents() {
        ...
        btndDelete = findViewById(R.id.btnDel)
    ...
    private fun initListeners() {
        ...
        btndDelete.setOnClickListener { btnDeleteClicked() }
    }
    ...
    /**
     * Borra el último dígito añadido o la última operación añadida
     */
    private fun btnDeleteClicked() {
        if (calc.numTemp2.isNotEmpty()) { // comprobamos si numTemp2 existe
            // borramos el último dígito añadido
            calc.numTemp2 = calc.numTemp2.substring(0, calc.numTemp2.length - 1)
            //actualizamos la información por pantalla
            if(calc.numTemp2 == ""){ // si se ha borrado numTemp2 entero, mostramos el operador y numTemp1
                muestraValor(calc.operadorTxt(),calc.numTemp1)
            }else { // si no mostramos numTemp2 y numTemp1 + el operador
                muestraValor(calc.numTemp2, calc.numTemp1 + calc.operadorTxt())
            }
        //comprobamos si se ha añadido el operador y si numTemp2 está vacío
        }else if(calc.op !=4 && calc.numTemp2 ==""){
            calc.op = 4 //asignamos al operador el valor ""
            calc.primerNum = true // asignamos el valor true a primerNum
            calc.numCalculos = 0 // reiniciamos el contador de cálculos
            //actualizamos la información por pantalla
            muestraValor(calc.numTemp1, calc.numTemp1)

        }else if (calc.numTemp1.isNotEmpty()) { //comprobamos si esta añadido el primer número o no
            // borramos el último dígito añadido
            calc.numTemp1 = calc.numTemp1.substring(0, calc.numTemp1.length - 1)
            //actualizamos la información por pantalla
            muestraValor(calc.numTemp1, calc.numTemp1)

        //si se ha borrado toda la información se lanza el mensaje de error
        }else if (txtDetalle.text.length == 0){
            mensajeError("No se puede borrar más")
        }
    }
}
